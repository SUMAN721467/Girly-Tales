...
export const DatabaseService = {
  ...
  async uploadCategoryCardImage(file: File): Promise<string> {
    // Store in 'banners/' folder in Supabase storage bucket as requested
    const fileToUpload = file.size > 10 * 1024 * 1024
      ? await this.optimizeImageFile(file, 2560, 0.96)
      : file;
    ...
    if (uploadError) {
      console.error('Supabase storage category card upload error:', uploadError);
      throw new Error(`Category card upload failed: ${uploadError.message}. Please verify the 'product-images' storage bucket exists in Supabase.`);
    }
